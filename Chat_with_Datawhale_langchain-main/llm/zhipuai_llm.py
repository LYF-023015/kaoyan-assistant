...
class ZhipuAILLM(Self_LLM):
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Note: ZhipuAI V2 python SDK currently does not natively support async stream.
        # Here we block on sync for now to avoid errors, or you can use standard wrapper.
        for res in self.client.chat.completions.create(**params):
            if res.choices[0].delta.content:
                chunk = GenerationChunk(text=res.choices[0].delta.content)
